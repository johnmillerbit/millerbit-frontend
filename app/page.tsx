  ...
  <section className="container mx-auto px-4 py-20 text-center">
    <div className="max-w-4xl mx-auto">
      ...
      <p className="text-xl text-blue-200 mb-8 max-w-2xl mx-auto font-light">
        {" "}
        {/* Adjusted text color */}
        MillerBit is a cutting-edge technology team specializing in AI
        solutions, web development, and innovative digital experiences that
    ...
    <div className="grid lg:grid-cols-2 gap-12 items-center">
      <div>
        ...
        <p className="text-slate-300 mb-6 text-lg">
          ...
          enthusiasts dedicated to creating innovative solutions that drive
          business growth and digital transformation.
        </p>
        <p className="text-slate-300 mb-8">
          {" "}
          {/* Adjusted text color */}
          Our expertise spans across multiple technologies and industries,
          allowing us to deliver comprehensive solutions that meet the unique
      ...
      <div className="relative">
        <div className="w-full h-96 bg-gradient-to-br from-blue-500/20 to-purple-500/20 rounded-2xl flex items-center justify-center border border-white/10">
          ...
          {/* Changed icon to Code2, adjusted color */}
        </div>
      </div>
    </div>
  </section>
    ...
    const fetchProjects = async () => {
      try {
        const response = await fetch(
          "http://localhost:5000/api/projects/portfolio"
        ); // Use the backend URL
        if (!response.ok) {
        ...
        const mappedProjects: Project[] = data.map((p: any) => ({
          ...
          project_picture_url: p.project_picture_url || "/placeholder.svg?height=300&width=400", // Ensure project_picture_url is always a valid string
          status: p.status,
          ...
          image:
            p.images.length > 0
              ? p.images[0]
              : "/placeholder.svg?height=300&width=400", // Use first image or placeholder
        }));
